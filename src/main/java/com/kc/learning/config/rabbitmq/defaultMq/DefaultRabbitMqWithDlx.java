...
import com.kc.learning.config.rabbitmq.BaseCustomizeMqWithDlx;
...
/**
 * 默认的带有死信队列的消息队列实例配置
 * todo 建议自定义带有死信队列的消息队列实例时统一类名：xxxRabbitMqWithDLX，
 * 同时类注解、类结构以及类静态变量名和default_mq/DefaultRabbitMqWithDLX保持一致，
 * 便于日志查看以及使用工具类，最简单的创建方式就是复制default_mq/DefaultRabbitMqWithDLX到customize_mq中全局修改文件中的“default”单词；
 *
 * @author stephen qiu
 */
@Slf4j
